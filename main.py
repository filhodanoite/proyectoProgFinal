...
def iniciar_juego(liga_seleccionada):
    global clubes, opciones, club_elegido, puntaje_total, vidas, vidas_generales, puntaje_acumulado
    resultado.config(text="")
    clubes = ligas_y_clubes[liga_seleccionada].copy()
    vidas = vidas_generales  # Usar vidas generales en lugar de reiniciarlas en cada juego
...
def reiniciar_juego():
    global puntaje_total, vidas_generales, puntaje_acumulado
    vidas_generales = 3  # Reiniciar las vidas generales
    puntaje_acumulado += puntaje_total
    puntaje_total = 0
    liga_var.set(liga_var.get())  # Establecer liga_var con el valor seleccionado
